...
occur on all types of printers.

One can check for a location bias by measuring the probe z_offset at
various X and Y locations. Ideally, the probe z_offset would be
constant for every printer location.
...
printer. For delta printers, move the head to a position near the A
tower and run the `PROBE_CALIBRATE` command; then to a position near
the B tower and run `PROBE_CALIBRATE`; then to a position near the C
tower. For cartesian, corexy, and similar printers, navigate to
positions near the four corners of the bed and run `PROBE_CALIBRATE`
at each position.

After each invocation of the `PROBE_CALIBRATE` command, note the
reported z_offset. Do not run `SAVE_CONFIG` after each measurement.
